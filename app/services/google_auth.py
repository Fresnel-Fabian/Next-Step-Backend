...
from app.utils.datetime_helpers import to_db, utc_now
...
async def exchange_google_code(
    ...
    drive_token_expiry is stored as naive UTC (to_db) for the DB column.
    """
    ...
    # workaround to timestamp package errors
    expires_in: int = token_data.get("expires_in", 3600)
    expiry = to_db(utc_now() + timedelta(seconds=expires_in))
...
async def get_or_create_google_user(db: AsyncSession, google_data: dict) -> User:
    ...
    def apply_drive_tokens(user: User) -> None:
        if google_data.get("drive_access_token"):
            user.drive_access_token = google_data["drive_access_token"]
        if google_data.get("drive_refresh_token"):
            user.drive_refresh_token = google_data["drive_refresh_token"]
        if google_data.get("drive_token_expiry"):
            user.drive_token_expiry = google_data["drive_token_expiry"]
    ...
    user = User(
        ...
        drive_access_token=google_data.get("drive_access_token"),
        drive_refresh_token=google_data.get("drive_refresh_token"),
        drive_token_expiry=google_data.get("drive_token_expiry"),
    )
    db.add(user)
    await db.commit()
    await db.refresh(user)
